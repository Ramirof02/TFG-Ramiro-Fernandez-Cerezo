"""
Experimento 9: Intermedios + Hard Mining
=========================================
Curriculum con SOLO niveles intermedios (3-7) + Hard Mining.
Fase actual + 30% más difíciles de fases anteriores.
...
Estructura:
  Fase 1: [3]
  Fase 2: [4] + 30% más difíciles de fase 1
  Fase 3: [5] + 30% más difíciles de fases 1-2
  Fase 4: [6] + 30% más difíciles de fases 1-3
  Fase 5: [7] + 30% más difíciles de fases 1-4
...
RESULTS_DIR = f"./resultados/exp9_results_{config.name}"
...
# Niveles intermedios por fase (solo 3-7)
PHASE_LEVELS = [
    [3],    # Fase 1: Más fácil de intermedios
    [4],    # Fase 2
    [5],    # Fase 3: Medio
    [6],    # Fase 4
    [7]     # Fase 5: Más difícil de intermedios
]
# ============================================================================
# MODELO CNN
# ============================================================================
...
def main():
    print("="*70)
    print(f"EXPERIMENTO 9: Intermedios + Hard Mining - {config.name}")
    print("="*70)
    ...
    print(f"   Validación: {len(val_dataset)} imágenes (todos los niveles)")
    print(f"    NOTA: Validación incluye niveles no entrenados (0-2, 8-9)")
    ...
    print("\nEstrategia: Niveles intermedios (3-7) + 30% difíciles anteriores (hard mining)")
    ...
    for phase in range(N_PHASES):
        ...
        if phase == 0:
            # Primera fase: solo datos actuales
            train_dataset = current_dataset
            print(f"Nivel actual: {current_levels}")
        else:
            # Fases 2+: añadir 30% más difíciles de fases anteriores
            previous_levels = []
            ...
            # Ordenar por dificultad (nivel) y tomar 30% más difíciles
            sorted_data = sorted(prev_dataset.data, key=lambda x: x['level'], reverse=True)
            hard_count = int(len(sorted_data) * 0.3)
            hard_data = sorted_data[:hard_count]
            
            # Crear dataset con difíciles
            ...
            hard_dataset = HardDataset(hard_data, transform)
            train_dataset = ConcatDataset([current_dataset, hard_dataset])
            
            print(f"Nivel actual: {current_levels}")
            print(f"+ 30% más difíciles de niveles anteriores: {previous_levels}")
    ...
    results = {
        'experiment': 'exp9_intermedios_hard_mining',
        'strategy': 'Intermedios (3-7) + 30% difíciles anteriores',
    ...
    print(f"\n NOTA: Validación incluye niveles no entrenados (0-2, 8-9)")
    print("="*70)

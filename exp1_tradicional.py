"""
Experimento 1: Entrenamiento Tradicional
========================================
Baseline: todos los datos mezclados sin curriculum learning.

ACTUALIZADO: Usa estructura train/val separada con rutas relativas
MULTI-DATASET: Soporta Digits y Cifar-10
...
RESULTS_DIR = f"./resultados/exp1_results_{config.name}"
...
class DigitCNN(nn.Module):
    """CNN para clasificación de dígitos - Adaptable a diferentes tamaños y canales"""
...
def main():
    print("="*70)
    print(f"EXPERIMENTO 1: Entrenamiento Tradicional (Baseline) - {config.name}")
    print("="*70)
    ...
    # Cargar TODOS los datos (todos los niveles)
    print("Cargando dataset...")
    train_dataset = LevelBasedDataset(TRAIN_DATASET, levels=None, transform=transform)
    ...
    # NUEVO: Detectar número de canales según dataset
    input_channels = 3 if config.name in ['cifar10rgb'] else 3  # Siempre 3 por el Grayscale(num_output_channels=3)
    
    model = DigitCNN(num_classes=NUM_CLASSES, input_size=IMAGE_SIZE, input_channels=input_channels).to(DEVICE)
    ...
    print("Estrategia: Sin curriculum (todos los datos mezclados)")
    print()
    ...
    results = {
        'experiment': 'exp1_tradicional',
        'strategy': 'Sin curriculum (baseline)',

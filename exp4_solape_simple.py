"""
Experimento 4: Curriculum con Overlap Simple
============================================
...
parser.add_argument(
    ...
    type=str,
    default='digits',
    choices=['digits', 'cifar10', 'cifar10rgb', 'mvtech_cable', 'mvtech_metalnut',
            'mvtech_transistor', 'mvtech_screw'],
    help='Dataset a usar'
)
...
RESULTS_DIR = f"./resultados/exp4_results_{config.name}"
...
class LevelBasedDataset(Dataset):
    ...
    - Mono-clase (MVTech): level_XX/ (imágenes directamente)
    """
...
def main():
    print("="*70)
    print(f"EXPERIMENTO 4: Curriculum con Overlap Simple - {config.name}")
    print("="*70)
    ...
    print("Estrategia: Fase actual + fase anterior completa")
    ...
    for phase in range(N_PHASES):
        ...
        # Niveles actuales + anteriores      
        levels = []         
        for p in range(phase + 1):  # Incluye fase actual             
            levels.extend(PHASE_LEVELS[p])  # ✓ Acumulativo
        
        print(f"Niveles: {levels}")
        
        train_dataset = LevelBasedDataset(TRAIN_DATASET, levels=levels, transform=transform)
    ...
    results = {
        'experiment': 'exp4_solape_simple',
        'strategy': 'Overlap: fase actual + fase anterior',

"""
Experimento 2: Curriculum por Etapas Temporales
===============================================
División temporal en 5 fases sin usar info de dificultad.

ACTUALIZADO: Usa estructura train/val separada

Ejecutar: python exp2_etapas_sin_dificultad.py
"""
...
RESULTS_DIR = f"./resultados/exp2_results_{config.name}"
...
class DigitCNN(nn.Module):
    """CNN para clasificación de dígitos - Optimizada para 128x128"""
...
def main():
    print("="*70)
    print(f"EXPERIMENTO 2: Etapas Temporales (Sin Info de Dificultad) - {config.name}")
    print("="*70)
    ...
    full_train = LevelBasedDataset(TRAIN_DATASET, levels=None, transform=transform)
    val_dataset = LevelBasedDataset(VAL_DATASET, levels=None, transform=transform)
    val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
    
    # Dividir aleatoriamente en 5 partes
    indices = list(range(len(full_train.data)))
    random.shuffle(indices)
    phase_size = len(indices) // N_PHASES
    
    print(f"   Train total: {len(full_train)} imágenes")
    print(f"   Val: {len(val_dataset)} imágenes")
    print(f"   Tamaño por fase: ~{phase_size} imágenes")
    ...
    print("Estrategia: División temporal aleatoria en 5 fases")
    print()
    ...
    for phase in range(N_PHASES):
        ...
        # Datos de esta fase
        phase_indices = indices[phase * phase_size : (phase + 1) * phase_size]
        phase_data = [full_train.data[i] for i in phase_indices]
        ...
        phase_dataset = PhaseDataset(phase_data, transform)
        phase_loader = DataLoader(phase_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=0)
        
        print(f"Entrenando con {len(phase_dataset)} imágenes")
        
        history['phase_info'].append({
            'phase': phase + 1,
            'n_samples': len(phase_dataset)
        })
        ...
        for epoch in range(phase_start, phase_end + 1):
            train_loss, train_acc = train_epoch(model, phase_loader, criterion, optimizer, DEVICE)
    ...
    results = {
        'experiment': 'exp2_etapas_sin_dificultad',
        'strategy': 'Etapas temporales aleatorias',
